...
(async () => {
	...
	let contract = new Contract('localhost', () => {
		...
		let dropdown = document.getElementById('flight-details');
		dropdown.length = 0;
		...
		DOM.elid('submit-oracle').addEventListener('click', () => {
			let flight = DOM.elid('flight-details').value;
			let departureDate = DOM.elid('departure-date').value;
		...
		DOM.elid('oracle-response').addEventListener('click', () => {
			...
			sleepFor(500).then(() => {
				...
				contract.submitOracleResponse(parseInt(eventIndex_), airline, flight, timestamps, (error, result) => {
					...
					DOM.elid('status-code').innerHTML = CODES[result.statusCode];
...
function display(title, description, results, id, cls) {
	...
	results.map((result) => {
		...
		if (id.toString() == "display-flight" && cls.toString() == "") {
			...
			b.setAttribute("data-target", "#purchaseInsuranceModal");
			row.appendChild(b);
...
function fetchOracleIndex(response) {
	const responseURL = 'http://localhost:3000/eventIndex';
